...
We crowdsourced relevance, diversity, and novelty judgments for the top one clarification question returned for each turn across all submitted runs for the Mixed Initiative sub task. In crowd-sourcing these judgements, we created the following key resources:

### Key resources: 

- MI Question Pool: All clarification questions evaluated for each turn. This consists of the top one question for each turn across all runs
- Updated MI Question Bank: All questions (from the initial question bank and generated by participant systems) used for the sub task and their IDs
- MI QRel Files: Aggregated judgements (relevance, diversity, novelty) for all questions in the MI Question Pool. These are also filtered for turns with one or more judgements greater than or equal to 2.
...
### Key resources: 
Response Pool: All responses evaluated for each turn. This consists of the top one response for each turn across all runs
Response Bank: All evaluated responses and their IDs
Response Runs:  Submitted runs converted to TREC run files for ease of evaluation
Response QRels: Aggregated judgements (relevance, conciseness, naturalness) for all questions in the Response Pool. These are filtered for turns that have one or more judgement greater than or equal to 2.
Description of draft pool file
